...
import projeto1.graficos.objetosGraficos.Caminho;
import projeto1.graficos.objetosGraficos.ElementosMapa;
import projeto1.graficos.objetosGraficos.Parede;
import projeto1.graficos.objetosGraficos.Porteira;
...
public class Mapa {
    ...
    private void corrigirRaioUnitario() {
        raio = (int) (height/(2*xMax));
    }
    
    public void refresh(int height) { 
        this.height = height;
        corrigirRaioUnitario();
       
        this.width = (int) ((yMax)*2*raio);
        
    }

    
    public ArrayList<ElementosMapa> criarMapa(TexturasGraficas textura) {
        ArrayList<ElementosMapa> e = new ArrayList();
        
        for(int i = 0; i < xMax; i++) {
            for(int j = 0; j < yMax; j++) {
                if(isParede(i, j) && !isPorteira(i, j)) {
                    e.add(new Parede(textura, j, i, raio));
                } else if(isPorteira(i, j)) { 
                    e.add(new Porteira(textura, j, i, raio));
                } else if(isCaminho(i, j)) {
                    e.add(new Caminho(textura, j, i, raio));
                }
            } 
        }
        
        for(ElementosMapa now: e) {
            if(now instanceof Parede) {
                int x = (int) now.getRealX();
                int y = (int) now.getRealY();
                boolean cima = false;
                if(isParede(y-1, x)) cima = true;
                
                boolean esquerda = false;
                if(isParede(y ,x-1)) esquerda = true;
                
                boolean direita = false;
                if(isParede(y, x+1)) direita = true;
                
                boolean baixo = false;
                if(isParede(y+1, x)) baixo = true;
                
                ((Parede)now).setBounds(cima, esquerda, baixo, direita);
            }
        }
        ...
        for(ElementosMapa now: e) {
            if(now instanceof Parede) {
                int x = (int) now.getRealX();
                int y = (int) now.getRealY();
                boolean cantoCimaEsq = false;
                if(isParede(y-1, x-1)) cantoCimaEsq = true;
                
                boolean cantoCimaDir = false;
                if(isParede(y-1 ,x+1)) cantoCimaDir = true;
                
                boolean cantoBaixoDir = false;
                if(isParede(y+1, x+1)) cantoBaixoDir = true;
                
                boolean cantoBaixoEsq = false;
                if(isParede(y+1, x-1)) cantoBaixoEsq = true;
                
                ((Parede)now).setCantos(cantoCimaEsq, cantoCimaDir, cantoBaixoDir, cantoBaixoEsq);
            }
        }
    ...
    public Ponto2f getRealPosition(int i, int j, float x0Table, float y0Table) {
        Ponto2f result = new Ponto2f();
        
        result.setX(x0Table+j*2*raio+raio);
        result.setY(y0Table+i*2*raio+raio);
        
        return result;
    }
    
    public Ponto2f getImaginaryPosition(float x, float y, float x0Table, float y0Table) {
        Ponto2f result = new Ponto2f();
        
        result.setY((int) Math.round((x-x0Table-raio)/(2*raio)));
        result.setX((int) Math.round((y-y0Table-raio)/(2*raio)));
    ...
    public ArrayList<Ponto> getPontosEPirulas(Tabuleiro tabuleiro) {
        ...
        for(int i = 0; i < xMax; i++) {
            for(int j = 0; j < yMax; j++) {
                if(isPonto(i, j)) {
                    pontos.add(new PacDots(i, j, tabuleiro));
                } else if(isPoder(i, j)) {
    ...
    public boolean isParede(int i, int j) {
        ...
        if(i < 0 || i >= xMax || j < 0 || j >= yMax) return false;
